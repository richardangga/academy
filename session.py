...
class session(models.Model):
	...
	instructur_id = fields.Many2one(comodel_name="res.partner",string="Instructur", required=True)
	...
	attendee_ids = fields.One2many(comodel_name="academic.attendee", inverse_name="session_id", string="Attendee", required=False)
	...
	@api.multi
	def _cek_instructur(self):
		for ession in self:
			x = [att.partner_id.id for att in session.attendee_ids]
			if session.instructur_id.id in x:
				return False

		return True

	_constraints = [(_cek_instructur, 'Instructor cannot be Attendee',['instructur_id', 'attendee_ids'])]
